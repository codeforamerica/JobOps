...
describe JobSearch do
  ...
  context "special methods" do
    ...
    it 'searches for keyword using search method' do
      job_search = Factory(:job_search, :keyword => "ruby")
      stub_request(:get, "http://www.jobcentral.com/api.asp?key=abc123&kw=ruby&zc=San%20Francisco,%20CA").to_return(:status => 200, :body => fixture("direct_employers_11b.xml"))
      stub_request(:get, "http://api.indeed.com/ads/apisearch?co=us&filter=1&format=json&fromage=1&highlight=0&jt=&l=San%20Francisco,%20CA&latlong=1&limit=30&publisher=xyz456&q=ruby&radius=25&sort=relevance&st=&useragent=Mozilla/4.0%20Firefox&userip=77.88.216.22&v=2").to_return(:status => 200, :body => fixture("indeed_ruby.json"))
      lambda {
        job_search.search
